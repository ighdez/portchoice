...
interactions = [[0,1]]
# Create model
obj = PortLogit(Y,X,None,C,B,interactions=interactions)

# Estimate
asc = np.array([1,1,1,1])
startv = np.zeros(asc.sum() + K + 1 + len(interactions))
fval, coef, se, hessian, diff_time = obj.estimate(startv,delta_0=None,asc=asc,tol=1e-6,verbose=True,hess=False)

# Construct results matrix
results = pd.DataFrame(np.c_[coef,se,coef/se],columns=['Estimate','Std.Err.','T-stat'],index=['ASC' + str(j+1) for j in range(asc.sum())] + ['Interaction'] + Xvars + ['Cost'])
