...
C = None#data[['Cost_' + str(i+1) for i in range(J)]]
X = data[[x + '_' + str(j+1) for j in range(J) for x in Xvars]]
B = None#3

# Create model
obj = PortLogit(Y,C,B,X)
...
results = pd.DataFrame(np.c_[coef,se,coef/se],columns=['Estimate','Std.Err.','T-stat'],index=['ASC' + str(j+1) for j in range(asc.sum())] + Xvars)
print(results)
exit()
